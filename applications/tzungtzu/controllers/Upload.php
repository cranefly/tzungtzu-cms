 ...
 * @author      Tz Dev Team
 */
class Upload extends TZ_Controller
{
    ...
    private function _check_md5()
    {
        ...
        if (isset($_FILES['file']['tmp_name'])){
            $md5content   = md5(file_get_contents($_FILES['file']['tmp_name']));
            $old_resource = $this->resource->find($md5content, 'content');
            
            if (!empty($old_resource)){
                $this->print_json(array('state' => 'SUCCESS', 'file_path' => $old_resource['url']));
            }
            unset($old_resource);
        }
        return $md5content;
    ...
    private function _action()
    {
        ...
        else
        {
			...
			$file_path	= $this->img_host . ltrim($file_url,'.');
