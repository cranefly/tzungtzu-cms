	...
	<form action="<?php echo base_url($this->PagePath);?>" method="post" id="form_data">
		...
		<table class="table_lists editbox">
			<tr>
				...
				<td><input type="text" name="data[flink_name]" class="comm_ipt" value="<?php echo $data['flink_name']?>"></td>
			</tr>
			<tr>
				<td class="fr"><span class="fred">* </span><?php echo $langcu['form_flink_url'] . $langco['colon']?></td>
				<td><input type="text" name="data[flink_url]" class="comm_ipt" value="<?php echo $data['flink_url']?>"> </td>
			</tr>
            <tr>
				<td class="fr"><?php echo $langcu['form_flink_img'] . $langco['colon']?></td>
                <td>
                    <input name="data[flink_img]" id="upload_img_value" value="<?php echo $data['flink_img']?>" type="hidden">
                    <div class="upload_img dz-clickable" id="upload_img">
                        <img src="<?php echo $data['flink_img']?>" style="height:30px;padding-top:5px; padding-left:5px;"></div>
			...
			<tr>
				...
				<td>
					<select name="data[group_id]" require="require">
						...
						<option value="<?php echo $group['id']?>" <?php if ($data['group_id'] == $data['group_id']) {echo 'selected="selected"';}?>><?php echo $group['gname']?></option>
						<?php }?>
					</select>
				</td>
			</tr>
